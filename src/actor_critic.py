from collections import deque

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim

from src.base_agent import BaseAgent


# --- 1. Define the Actor Network (Policy) ---
# The Actor decides which action to take given a state.
# For a discrete action space, it outputs probabilities over actions.
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_size=128):
        super(Actor, self).__init__()
        # Define layers for the actor network
        self.fc1 = nn.Linear(state_dim, hidden_size)
        self.fc2 = nn.Linear(hidden_size, action_dim)

    def forward(self, state):
        # Forward pass through the network
        x = F.relu(self.fc1(state))  # Apply ReLU activation to the first layer
        # Output log probabilities of actions (for numerical stability with CrossEntropyLoss)
        return F.log_softmax(self.fc2(x), dim=-1)


# --- 2. Define the Critic Network (Value Function) ---
# The Critic estimates the value (expected return) of being in a given state.
class Critic(nn.Module):
    def __init__(self, state_dim, hidden_size=128):
        super(Critic, self).__init__()
        # Define layers for the critic network
        self.fc1 = nn.Linear(state_dim, hidden_size)
        self.fc2 = nn.Linear(hidden_size, 1)  # Output a single value (state-value)

    def forward(self, state):
        # Forward pass through the network
        x = F.relu(self.fc1(state))  # Apply ReLU activation
        return self.fc2(x)  # Output the state value


# --- 3. Define the Actor-Critic Agent ---
# This class combines the Actor and Critic, handles training, and action selection.
class ActorCriticAgent(BaseAgent):
    def __init__(
        self, state_dim, action_dim, lr_actor=1e-3, lr_critic=1e-3, gamma=0.99
    ):
        # Initialize actor and critic networks
        self.actor = Actor(state_dim, action_dim)
        self.critic = Critic(state_dim)

        # Define optimizers for both networks
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=lr_actor)
        self.critic_optimizer = optim.Adam(self.critic.parameters(), lr=lr_critic)

        self.gamma = gamma  # Discount factor for future rewards
        self.log_probs = []  # To store log probabilities of actions taken
        self.rewards = []  # To store rewards received
        self.values = []  # To store state values predicted by the critic
        self.dones = []  # To store done flags

    def select_action(self, state, greedy=False):
        # Convert state (numpy array) to PyTorch tensor
        state = torch.from_numpy(state).float().unsqueeze(0)
        # Get log probabilities from the actor
        log_probs = self.actor(state)
        # Create a categorical distribution from log probabilities
        dist = torch.distributions.Categorical(logits=log_probs)

        if greedy:
            action = torch.argmax(log_probs, dim=-1)
        else:
            # Sample an action from the distribution
            action = dist.sample()

        # Store the log probability of the chosen action and the predicted value
        self.log_probs.append(dist.log_prob(action))
        self.values.append(self.critic(state))

        # Return the action as a numpy integer
        return action.item()

    def store_transition(self, reward, done):
        # Store the reward and done flag for the current transition
        self.rewards.append(reward)
        self.dones.append(done)

    def learn(self):
        # Calculate discounted rewards (returns)
        returns = []
        R = 0
        # Iterate backwards to calculate discounted returns
        for i in reversed(range(len(self.rewards))):
            if self.dones[i]:  # If episode ended, reset R
                R = 0
            R = self.rewards[i] + self.gamma * R
            returns.insert(0, R)  # Insert at the beginning to maintain order

        # Convert lists to PyTorch tensors
        returns = torch.tensor(returns, dtype=torch.float32)
        log_probs = torch.cat(self.log_probs)
        values = torch.cat(self.values).squeeze()  # Remove extra dimension

        # Normalize returns (optional, but often helps stability)
        returns = (returns - returns.mean()) / (returns.std() + 1e-9)

        # Calculate Advantage: TD Error = G_t - V(S_t)
        # Advantage is the difference between the actual return and the critic's prediction
        advantage = returns - values

        # --- Critic Loss ---
        # Mean Squared Error between predicted values and actual returns
        critic_loss = F.mse_loss(values, returns)

        # --- Actor Loss ---
        # Actor aims to maximize expected return, so we minimize -log_prob * advantage
        # The negative sign is because optimizers minimize loss.
        actor_loss = -(
            log_probs * advantage.detach()
        ).mean()  # .detach() prevents gradients flowing to critic

        # --- Update Networks ---
        # Zero gradients, perform backward pass, and update weights for critic
        self.critic_optimizer.zero_grad()
        critic_loss.backward()
        self.critic_optimizer.step()

        # Zero gradients, perform backward pass, and update weights for actor
        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        self.actor_optimizer.step()

        # Clear stored experience for the next episode
        self.log_probs = []
        self.rewards = []
        self.values = []
        self.dones = []

    def save(self, path):
        torch.save(
            {
                "actor_state_dict": self.actor.state_dict(),
                "critic_state_dict": self.critic.state_dict(),
            },
            path,
        )

    def load(self, path):
        checkpoint = torch.load(path)
        self.actor.load_state_dict(checkpoint["actor_state_dict"])
        self.critic.load_state_dict(checkpoint["critic_state_dict"])


# --- 4. Training Loop ---
def train(
    env, state_dim, action_dim, num_episodes, max_steps_per_episode, target_score
):
    # Initialize the Actor-Critic agent
    agent = ActorCriticAgent(state_dim, action_dim)

    scores_deque = deque(maxlen=100)  # To store recent scores for average calculation
    scores = []  # To store all scores

    print("Starting Actor-Critic training...")

    for episode in range(1, num_episodes + 1):
        state, _ = env.reset()  # Reset environment for a new episode
        episode_reward = 0

        for step in range(max_steps_per_episode):
            action = agent.select_action(state)  # Agent selects an action
            next_state, reward, terminated, truncated, _ = env.step(
                action
            )  # Environment takes a step
            done = terminated or truncated  # Check if episode is done

            agent.store_transition(reward, done)  # Store the transition

            state = next_state  # Update current state
            episode_reward += reward  # Accumulate reward

            if done:  # If episode finished
                break

        # After each episode, the agent learns from the collected experience
        agent.learn()

        scores_deque.append(episode_reward)  # Add current episode reward to deque
        scores.append(episode_reward)  # Add to all scores

        # Print progress
        if episode % 100 == 0:
            print(f"Episode {episode}\tAverage Score: {np.mean(scores_deque):.2f}")

        # Check if the environment is solved (average score over 100 episodes is 475 or more)
        if np.mean(scores_deque) >= target_score:
            print(
                f"\nEnvironment solved in {episode} episodes! Average Score: {np.mean(scores_deque):.2f}"
            )
            break

    print("\nTraining complete.")

    return agent, scores
