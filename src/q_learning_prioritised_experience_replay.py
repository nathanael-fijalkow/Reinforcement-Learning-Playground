...
class ReplayBuffer:
    def __init__(self, capacity):
        ...
        self.Bias = deque([], maxlen=capacity)

    def push(self, bias, *args):
        """Save a transition"""
    ...
    def sample(self):
        ...
        weights = [float(np.ravel(b)[0]) for b in self.Bias]
...
def train(env, state_dim, action_dim, num_episodes, max_steps_per_episode, target_score):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps_per_episode):
            ...
            max_arg = np.argmax(agent.q_table[next_state])

            bias = episode_reward + agent.gamma * agent.q_table[max_arg] - agent.q_table[state, action]

            agent.memory.push(bias, state, action, reward, next_state, done)
        ...
        for i in range(10*max_steps_per_episode):
            agent.learn()
