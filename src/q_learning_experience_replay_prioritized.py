...
class PrioritizedReplayBuffer:
    ...
    def push(self, *args, bias):
        if len(self.memory) < self.capacity:
            self.memory.append(Transition(*args))
            self.biases.append(bias)
        else:
            self.memory[self.position] = Transition(*args)
            self.biases[self.position] = bias
    ...
    def sample(self):
        ...
        probabilities = exp_biases / np.sum(exp_biases)
...
def train(env, state_dim, action_dim, num_episodes, max_steps_per_episode, target_score):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps_per_episode):
            ...
            bias = reward + agent.gamma * next_max - old_value

            agent.memory.push(state, action, reward, next_state, done, bias=bias)
            agent.learn()
