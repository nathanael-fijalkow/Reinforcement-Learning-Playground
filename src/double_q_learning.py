...
class DoubleQLearningAgent(BaseAgent):
    ...
    def select_action(self, state, use_q1=True, greedy=False):
    ...
    def learn(self, state, action, reward, next_state, done, update_q1=True):
        if update_q1:
            old_value = self.q_table1[state, action]
            next_action = np.argmax(self.q_table2[next_state, :])

            new_value = (1 - self.lr) * old_value + \
                        self.lr * (reward + self.gamma * self.q_table1[next_state, next_action])
            ...
            new_value = (1 - self.lr) * old_value + \
                        self.lr * (reward + self.gamma * self.q_table2[next_state, next_action])
...
def train(env, state_dim, action_dim, num_episodes, max_steps_per_episode, target_score):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps_per_episode):
            q1 = True if np.random.rand() >= 0.5 else False
            
            action = agent.select_action(state, use_q1=q1)
            next_state, reward, terminated, truncated, _ = env.step(action)
            done = terminated or truncated

            agent.learn(state, action, reward, next_state, done, update_q1=q1)
